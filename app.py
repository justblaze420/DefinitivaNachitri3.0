...
def loginPage():
    if request.method == 'POST':
        ...
        try:
            ...
            if user and user['contrasena'] == passwd:
...
def createBooks():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('insertLibro', [nombre, autor, año_publicacion, stock, genero, fecha_pub, estado])
        conn.commit()
...
def updateBooks():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('updateLibro', [book_id, nombre, autor, año_publicacion, stock, genero, fecha_pub, estado])
        conn.commit()
...
def deleteBooks(id):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('deleteLibro', [nombre, id]) 
        conn.commit()
